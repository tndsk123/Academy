...
import community_exchange.dao.Community_exchangeDAO;
import community_exchange.dto.Community_exchangeDTO;
import community_exchange.dto.Community_exchange_commentDTO;
import config.Pager;


@WebServlet("/community_exchange_servlet/*")
public class Community_exchangeController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String url=request.getRequestURL().toString();
		String contextPath=request.getContextPath();
		Community_exchangeDAO dao=new Community_exchangeDAO();
		...
		if(url.indexOf("list.do")!=-1) { 
			...
			List<Community_exchangeDTO> list=dao.list(start,end,list_view);
			request.setAttribute("list", list);
			request.setAttribute("page", pager);
			request.setAttribute("list_view", list_view);
			String page="/community_exchange.jsp";
			...
			Community_exchangeDTO dto=new Community_exchangeDTO();
			...
			dao.insert(dto);
			String page="/community_exchange_servlet/list.do";
			...
			Community_exchangeDTO dto=dao.viewReplace(num);
			request.setAttribute("count", count);
			request.setAttribute("dto", dto);
			String page="/community_exchange_view.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		} else if(url.indexOf("commentList.do")!=-1) {
			System.out.println("댓글 리스트 실행");
			int num=Integer.parseInt(request.getParameter("num"));
			List<Community_exchange_commentDTO> list=dao.commentList(num);
			request.setAttribute("list", list);
			String page="/community_exchange_comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		} else if(url.indexOf("comment_add.do") != -1) {
			Community_exchange_commentDTO dto=new Community_exchange_commentDTO();
			//게시물 번호
			...
			String page="";
			if(result != null) {
				page="/community_exchange_list_edit.jsp";
				request.setAttribute("dto", dao.view(num));
				RequestDispatcher rd=request.getRequestDispatcher(page);
				rd.forward(request, response);
			}else {//비밀번호가 틀리면
				page=contextPath+"/community_exchange_servlet/view.do?num="
			+num+"&message=error";
				response.sendRedirect(page);
			}//else
		} else if(url.indexOf("community_exchange_update.do")!=-1) {
			File uploadDir=new File(Constants.UPLOAD_PATH);
			if(!uploadDir.exists()) {
				uploadDir.mkdir();
			}
			...
			dto.setIp(ip);
			if(filename == null || filename.trim().equals("")) {
				Community_exchangeDTO dto2=dao.view(num);
			...
			String page="/community_exchange_servlet/list.do";
			response.sendRedirect(contextPath+page);
		}else if(url.indexOf("community_exchange_delete.do")!=-1) {
			MultipartRequest multi=new MultipartRequest(request,Constants.UPLOAD_PATH, Constants.MAX_UPLOAD, "utf-8", new DefaultFileRenamePolicy());
			int num=Integer.parseInt(multi.getParameter("num"));
			dao.delete(num);
			//페이지 이동
			...
			List<Community_exchangeDTO> list=dao.searchList(keyword);
