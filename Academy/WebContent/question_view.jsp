...
$(function(){
	...
	$("#btnList").click(function(){
		location.href="${path}/question_servlet/list.do";
	});
	$("#btnEdit").click(function(){
		...
		document.form1.action="${path}/question_servlet/pass_check.do";
		document.form1.submit();
	});
	$("#reply").click(function(){
		var accesswrite=$("#accesswrite").val();
		if(accesswrite == '관리자'){
		document.form1.action="${path}/question_servlet/reply.do";
		document.form1.submit();
		}else{
			alert('관리자에게만 허용된 권한입니다.');
			return;
		}
	});
...
function comment_list(){
	$.ajax({
		type: "post",
		url: "${path}/question_servlet/commentList.do",
		data: "num=${dto.num}",
...
function comment_add(){
	...
	$.ajax({
		type: "post",
		url: "${path}/question_servlet/comment_add.do",
		data: param,
		success: function(){
			$("#co_content").val("");
			comment_list();
...
function prePage() {
	...
	location.href="${path}/question_servlet/view.do?num="+num;
}
function nextPage() {
	var num=${dto.num}-1;
	if(num<=0){
      		 ...
      		 <div class="col-sm-12 mt-2">
      		 	...
      		 	<table class="table table-bordered" >
  					...
  					<tr>
    					...
    					<td colspan="3">
      						<c:if test="${dto.filesize > 0}">
        						...
        						<a href="${path}/question_servlet/download.do?num=${dto.num}">[다운로드]</a>
      						</c:if>
    					</td>
  					</tr>		
  				  </table>
  				  <div class="col-sm-12 mt-3"  align="center">
  				  	...
  				  	<button type="button" class="btn btn-primary mt-3 ml-4" id="reply">답변달기</button>
