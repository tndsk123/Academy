...
$(function (){
	$("#btnJoin").click(function() {
		...
		var idchecksucess=$("#idchecksucess").val();
		var nicknamechecksucess=$("#nicknamechecksucess").val();
		if(id==""){
			alert("아이디를 입력해주세요");
			userid.focus();
			return;
...
function idCheck() {
	var id=$("#id").val();
	document.form1.action="${path}/userInfo_servlet/idcheck.do";
	document.form1.submit();
}

function nicknameCheck() {
	var nickname=$("#nickname").val();
	document.form1.action="${path}/userInfo_servlet/nicknamecheck.do";
	document.form1.submit();
}
...
<body>
	...
	<section class="container mt-3" style="max-width: 560px;">
		...
		<form method="post" name="form1" id="form1" action="${path}/userInfo_servlet/join.do">
			<div class="form-group">
				<label>아이디</label>
				<% String id=(String)session.getAttribute("id"); 
				if(id==null){%>
				<input name="id" id="id" class="form-control" placeholder="아이디를 입력하세요.">
				<button class="btn btn-primary mt-2" onclick="idCheck()">중복체크</button>
				<%}else{ %>
				<input name="id" id="id" class="form-control" value="<%=id%>" disabled="disabled">
				<%} %>
				<% String result=(String)session.getAttribute("result"); 
				if(result!=null){%>
				<span class="ml-2 mt-2" style="color: blue"><%=result%></span>
				<input type="hidden" name="idchecksucess" id="idchecksucess" value="<%=result%>">
				<%}else{%>
				<input type="hidden" name="idchecksucess" id="idchecksucess" value="">
				<%} %>
			</div>
			<div class="form-group">
				<label>비밀번호</label>
			...
			<div class="form-group">
				...
				<% 
				if(nickname==null){%>
				<input name="nickname" id="nickname" class="form-control" placeholder="닉네임을 입력하세요.">
				<button class="btn btn-primary mt-2" onclick="nicknameCheck()">중복체크</button>
				<%}else{ %>
				<input name="nickname" id="nickname" class="form-control" value="<%=nickname%>" disabled="disabled">
				<%} %>
				<% String result2=(String)session.getAttribute("result2"); 
				if(result2!=null){%>		
				<span id="nicknameCheckResult" class="ml-2 mt-2" style="color: blue"><%=result2%></span>
				<input type="hidden" name="nicknamechecksucess" id="nicknamechecksucess" value="<%=result2%>">
				<%}else{%>
				<input type="hidden" name="nicknamechecksucess" id="nicknamechecksucess" value="">
				<%} %>
			</div>
			<div class="form-group">
				<label>이메일</label>
				<input type="email" id="email" name="email" class="form-control" placeholder="이메일을 입력하세요.">
				<span class="ml-2 mt-2" style="color: blue">이메일 인증 가능한 메일을 입력해주세요.</span>
			</div>
			<div class="form-group">
				<label>전화번호</label>
				<input type="text" id="hp" name="hp" class="form-control" placeholder="000-0000-0000">
			</div>
			<div class="form-group">
				<label>생년월일</label>
			...
			<div class="form-group" align="center">
				<a class="btn btn-danger mx-1 mt-2" data-toggle="modal" href="#termsUseModal">약관보기</a>	
				<button type="button" class="btn btn-primary mx-1 mt-2" id="btnJoin">회원가입</button>
			</div>
		</form>
	</section>
	<div class="modal fade" id="termsUseModal" tabindex="-1" role="dialog" aria-labelledby="modal" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="modal">이용약관</h5>
				...
				<div class="modal-body">
					<form action="" id="form2" name="form2" method="post">
						<div class="form-group">
							<label>약관 제목</label>
							<textarea name="termsSubject" class="form-control" maxlength="2048" style="height: 180px;">
제1조(목적) 

이 약관은 Mobile Academy 회사(전자상거래 사업자)가 운영하는 Mobile Academy 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.
  ...
  ※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다.」
						...
						<div class="form-group">
							<label>약관 내용</label>
							<textarea name="termsContent" class="form-control" maxlength="2048" style="height: 180px;">
제2조(정의)

  ① “몰”이란 Mobile Academy 회사가 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
제24조(재판권 및 준거법)
  ...
  ② “몰”과 이용자 간에 제기된 전자상거래 소송에는 한국법을 적용합니다.
						...
						<input type="radio" name="termsCheck" id="termsCheck">약관에 동의합니다.
