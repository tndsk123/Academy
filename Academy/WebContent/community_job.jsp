...
function list_change(page) {
	...
	location.href="${path}/community_job_servlet/list.do?curPage="+page+"&sequence="+sequence;	
}
function list_view() {
	document.list.action="${path}/community_job_servlet/list.do";
		...
		<ul class="navbar-nav ml-auto">
			...
			<li class="nav-item dropdown">
				...
				<div class="text-center dropdown-menu" aria-labelledby="dropdown">
					...
					<a class="dropdown-item mt-2" href="${path}/${path}/logout.jsp">로그아웃</a>
				</div>
			<%} %>
			</li>
			<li class="nav-item">
      		 		...
      		 		<form method="post" name="list" action="">
      		 			<select onchange="list_view()" style="width: 200px" name="sequence" id="sequence">
      		 				<c:choose>
      		 					...
      		 					<c:when test="${list_view == 'good'}">
      		 				 		<option disabled selected="selected" value="good">추천순</option>
      		 					</c:when>
      		 					<c:otherwise>
      		 		...
      		 		<c:forEach var="dto" items="${list}">
      		 		    <c:choose>
      		 		    	<c:when test="${dto.writer == '관리자' }">
      		 						...
      		 						<td>${dto.job_area}</td>
      		 						<td>
      		 							<a href="${path}/community_job_servlet/view.do?num=${dto.num}" style="color: blue;">${dto.subject}</a>
      		 						</td>
      		 						<td>${dto.readcount}</td>   		 						
      		 						<td>${dto.good}</td>
      		 						<td align="center">
      									<c:if test="${dto.filesize > 0}">
      										<a href="${path}/community_job_servlet/download.do?num=${dto.num}" style="color: black;">
